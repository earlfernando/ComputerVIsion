...
for i =1 : number_of_objects   
   for j = 1: number_of_images
    ...
    [model,inlierIndices,outlierIndices] = pcfitplane(ptCloudIn,maxDistance,'MaxNumTrials',10000,'Confidence',99);
    inliers_2d=normalized2Dpoint(:,inlierIndices);
    inliers_3d=normalized3Dpoint(:,inlierIndices);
       for k=1:iterations
            ind = randsample(size(normalized3Dpoint,2),3);
...
%                   intial_3d_points = {normalized3Dpoint};
                 ...
                 for r =1:numel(Ps) 
                     temp_ones = ones(1,size(normalized3Dpoint,2));
                  ...
                  %%% end of implementation
                     ...
                     num_inlier = 0;
                    for h =1:size(inliers_2d,2)
                      local_3d_point = [inliers_3d(:,h);1];
                      %  local_3d_point = inliers_3d(:,h);
                       local_2d_point = inliers_2d(:,h);
...
for i =1:number_of_images
    ...
    average = average+cell2mat(scores);
end
average =average/7;
