...
hold off
pointss = cell2mat(x(1));;

number_of_points = size(pointss,2);

new_points_new = pflat(P_1*([Xmodel;ones(1,size(Xmodel,2))]));
figure(12)
img = imread('cube1.JPG');
imshow(img)
hold on
plot(pointss(1,1:number_of_points),pointss(2,1:number_of_points),'ro','Markersize',10)
plot(new_points_new(1,1:number_of_points),new_points_new(2,1:number_of_points),'+g','Markersize',10)



pointss = cell2mat(x(2));
number_of_points = size(pointss,2);

new_points_new = pflat(P_2*([Xmodel;ones(1,size(Xmodel,2))]));
figure(11)
